...
void servo()
{
  for(int pos = 0; pos < 180; pos += 1)  // cho servo quay từ 0->179 độ
  {                                  // mỗi bước của vòng lặp tăng 1 độ
    myservo.write(pos);              // xuất tọa độ ra cho servo
    delay(5);                       // đợi 15 ms cho servo quay đến góc đó rồi tới bước tiếp theo
  } 
  delay(500);  
  
  for(int pos = 180; pos > 0; pos --)     // cho servo quay từ 179-->0 độ
  {                                
    myservo.write(pos);              // xuất tọa độ ra cho servo
    delay(5);                       // đợi 15 ms cho servo quay đến góc đó rồi tới bước tiếp theo
  } 
